  ...
  try {
    ...
    let htmlContent = `
      <html>
        ...
        <body>
          <div class="header">
            <h1>PI HEALTH Report Summary</h1>
    ...
    htmlContent += `
          <div class="footer">
            ...
            <p>Generated by PI HEALTH • Keep this report safe</p>
          </div>
        </body>
      </html>
    `;
    ...
    const fileName = `PI HEALTH_Reports_${new Date().toISOString().split('T')[0]}.pdf`;
...
export async function generateDOCXReport(reports: LabReport[]): Promise<string> {
  try {
    ...
    sections.push(
      ...
      new Paragraph({
        text: 'Generated by PI HEALTH • Keep this report safe',
        alignment: AlignmentType.CENTER,
      })
    );
    ...
    const fileName = `PI HEALTH_Reports_${new Date().toISOString().split('T')[0]}.docx`;
  ...
  try {
    ...
    const htmlContent = `
      <html>
        ...
        <body>
          <div class="header">
            <h1>PI HEALTH Radiology Analysis</h1>
          ...
          <div class="footer">
            <p>PI HEALTH Radiology Assistant</p>
    ...
    const fileName = `PI HEALTH_Radiology_${safeExam}_${dateStr}.pdf`;
    const newPath = `${FileSystem.documentDirectory}${fileName}`;
...
export async function generateRadiologyDOCX(
  ...
  try {
    ...
    sections.push(
      new Paragraph({
        text: 'PI HEALTH Radiology Analysis',
        heading: HeadingLevel.HEADING_1,
        alignment: AlignmentType.CENTER,
        spacing: { after: 200 },
      }),
    ...
    const safeExam = examType.replace(/\s+/g, '');
    const dateStr = new Date().toISOString().split('T')[0];
    const fileName = `PI HEALTH_Radiology_${safeExam}_${dateStr}.docx`;
    const filePath = `${FileSystem.documentDirectory}${fileName}`;
  ...
  try {
    if (await Sharing.isAvailableAsync()) {
      await Sharing.shareAsync(filePath, {
        ...
        dialogTitle: 'Share PI HEALTH Report',
        UTI: getUTI(filePath),
      });
    } else {

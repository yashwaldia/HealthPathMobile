...
export default function HistoryScreen() {
  ...
  const fetchReports = async () => {
    if (!user?.uid) return;
    try {
      const reportsRef = collection(db, `users/${user.uid}/weeklyReports`);
      const q = query(reportsRef, orderBy('weekStartDate', 'desc'));
      const snapshot = await getDocs(q);
      
      const fetchedReports = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      } as WeeklyReportSummary));
    ...
    >
      <View style={styles.cardHeader}>
        <View style={styles.dateBadge}>
          ...
          <Text style={styles.dateText}>{formatDateRange(item.weekStartDate, item.weekEndDate)}</Text>
        </View>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.statItem}>
          <Ionicons name="document-text" size={16} color={Colors.light.warning} />
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <Ionicons name="time-outline" size={80} color={Colors.light.border} />
              <Text style={styles.emptyText}>No weekly reports yet.</Text>
              <Text style={styles.emptySubText}>Your AI health summaries will appear here every Monday.</Text>
            </View>
          }
